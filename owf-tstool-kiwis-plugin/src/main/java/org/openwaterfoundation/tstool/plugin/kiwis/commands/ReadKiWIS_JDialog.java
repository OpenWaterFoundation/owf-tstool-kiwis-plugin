// ReadKiWIS_JDialog - editor for the ReadKiWIS() command.
...
private ReadKiWIS_Command __command = null;
...
private List<KiWIS_TimeSeries_InputFilter_JPanel> __inputFilterJPanelList = new ArrayList<>();
private KiWISDataStore __dataStore = null; // Selected KiWISDataStore.
...
*/
public ReadKiWIS_JDialog ( JFrame parent, ReadKiWIS_Command command ) {
	super(parent, true);
...
/**
Refresh the data type choices in response to the currently selected KiWIS datastore.
...
/**
Refresh the query choices for the currently selected KiWIS datastore.
@param value if non-null, then the selection is from the command initialization, in which case the
specified data type should be selected
*/
...
/**
Refresh the query choices for the currently selected KiWIS location identifier.
@param value if non-null, then the selection is from the command initialization, in which case the
specified data type should be selected
*/
private void actionPerformedLocIdSelected ( ) {
    ...
    // Populate the time series short name choices corresponding to the location identifier.
    populateTsShortNameChoices ( getSelectedDataStore(), getSelectedDataType(), getSelectedLocId() );
}
...
private void commitEdits () {
	...
	String TsShortName = __TsShortName_JComboBox.getSelected();
	__command.setCommandParameter ( "TsShortName", TsShortName );
	// 1+ time series.
	String delim = ";";
	//InputFilter_JPanel filterPanel = getVisibleInputFilterPanel();
...
*/
private KiWISDataStore getDataStore() {
    return __dataStore;
}
...
private List<KiWIS_TimeSeries_InputFilter_JPanel> getInputFilterJPanelList () {
...
*/
private KiWISDataStore getSelectedDataStore () {
	String routine = getClass().getSimpleName() + ".getSelectedDataStore";
    String DataStore = __DataStore_JComboBox.getSelected();
    KiWISDataStore dataStore = (KiWISDataStore)((TSCommandProcessor)
        __command.getCommandProcessor()).getDataStoreForName( DataStore, KiWISDataStore.class );
    if ( dataStore != null ) {
...
private String getSelectedDataType() {
    ...
    // Make sure to use spaces around the dashes because dash without space is used to indicate statistic, and want that
    // included in the data type.
...
private String getSelectedLocId() {
    ...
    return __LocId_JComboBox.getSelected();
}
...
private KiWIS_TimeSeries_InputFilter_JPanel getVisibleInputFilterPanel() {
    //List<InputFilter_JPanel> panelList = getInputFilterJPanelList();
    List<KiWIS_TimeSeries_InputFilter_JPanel> panelList = getInputFilterJPanelList();
    String panelName;
    //for ( InputFilter_JPanel panel : panelList ) {
    for ( KiWIS_TimeSeries_InputFilter_JPanel panel : panelList ) {
        // Skip default.
...
@param command Command to edit.
*/
private void initialize ( JFrame parent, ReadKiWIS_Command command ) {
    ...
    JGUIUtil.addComponent(main_JPanel, new JLabel (
    	"Read 1+ time series from a KiWIS web services datastore, using options from the choices below to select time series."),
        0, ++y, 7, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
   	JGUIUtil.addComponent(main_JPanel, new JLabel (
		"Refer to the KiWIS web services documentation for information about data types (stationparameter_no) and intervals (ts_spacing)." ),
		0, ++y, 7, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
   	JGUIUtil.addComponent(main_JPanel, new JLabel (
    ...
    Message.printStatus(2, "ReadKiWIS", "Getting datastores for KiWISDataStore class");
    List<DataStore> dataStoreList = tsProcessor.getDataStoresByType( KiWISDataStore.class );
    ...
    __tsInfo_JTabbedPane.setBorder(
        ...
        "Indicate how to match time series in KiWIS" ));
    ...
    JGUIUtil.addComponent(singleTS_JPanel, new JLabel ( "Data source:"),
        0, ++ySingle, 1, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.EAST);
    __DataSource_JTextField = new JTextField ( "KiWIS", 20 );
    __DataSource_JTextField.setToolTipText("Data source to match (currently always 'KiWIS').");
    ...
    JGUIUtil.addComponent(singleTS_JPanel, __DataSource_JTextField,
        1, ySingle, 2, 1, 1, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
    JGUIUtil.addComponent(singleTS_JPanel, new JLabel ( "Used in TSID, defaults to 'KiWIS'."),
        3, ySingle, 2, 1, 0, 0, insetsTLBR, GridBagConstraints.NONE, GridBagConstraints.WEST);
...
/**
Initialize input filters for all of the available KiWIS datastores.
...
@param dataStoreList the list of available KiWISDataStore
*/
private void initializeInputFilters ( JPanel parent_JPanel, int y, List<DataStore> dataStoreList ) {
    ...
    for ( DataStore ds : dataStoreList ) {
    	Message.printStatus(2,routine,"Initializing data store list for datastore name \"" + ds.getName() +
    		"\" class: " + ds.getClass() );
    	Message.printStatus(2, routine, "Casting to KiWISDataStore class: " + KiWISDataStore.class);
        initializeInputFilters_OneFilter ( parent_JPanel, y, (KiWISDataStore)ds);
    }
    ...
    List<KiWIS_TimeSeries_InputFilter_JPanel> ifPanelList = getInputFilterJPanelList();
    //InputFilter_JPanel panel = new InputFilter_JPanel("Data type and interval have no input filters.");
    KiWIS_TimeSeries_InputFilter_JPanel panel =
    	new KiWIS_TimeSeries_InputFilter_JPanel("Data type and interval have no input filters.");
...
*/
private void initializeInputFilters_OneFilter ( JPanel parent_JPanel, int y, KiWISDataStore dataStore ) {
    ...
    List<KiWIS_TimeSeries_InputFilter_JPanel> inputFilterJPanelList = getInputFilterJPanelList();
    ...
    try {
        // Time series...
        KiWIS_TimeSeries_InputFilter_JPanel panel = new KiWIS_TimeSeries_InputFilter_JPanel ( dataStore, 5 );
        ...
        "Unable to initialize input filter for KiWIS time series catalog (" + e + ")." );
...
*/
private void populateIntervalChoices ( KiWISDataStore datastore ) {
...
private void populateLocIdChoices ( KiWISDataStore datastore ) {
	...
	//boolean includeWildcards = false;
    List<String> locIds0 = datastore.getLocIdStrings(getSelectedDataType());
...
/**
Set the time series short name choices in response to a new location ID being selected.
The location choices are needed when reading a single time series.
@param datastore the datastore to use to determine the data types
*/
private void populateTsShortNameChoices ( KiWISDataStore datastore, String selectedDataType, String selectedLocId ) {
	if ( datastore == null ) {
		return;
	}
	//boolean includeWildcards = false;
    List<String> tsShortNames0 = datastore.getTsShortNameStrings(selectedDataType, selectedLocId);
    // Make a copy so that the global list is not changed.
    List<String> tsShortNames = new ArrayList<>();
    for ( String tsShortName : tsShortNames0 ) {
    	tsShortNames.add(tsShortName);
    }
    // Add a blank because multiple time series tab might be used.
    tsShortNames.add(0,"");
    __TsShortName_JComboBox.setData ( tsShortNames );
    // Select the default.
    __TsShortName_JComboBox.select(0);
}
...
private void refresh () {
	...
	if ( __first_time ) {
        ...
        populateLocIdChoices(getSelectedDataStore());
        // Now select what the command had previously (if specified).
        //if ( JGUIUtil.isSimpleJComboBoxItem(__LocId_JComboBox, LocId, JGUIUtil.CHECK_SUBSTRINGS, "-", 1, index, true ) ) {
        ...
        // Populate the time series short name (ts_shortname) choices based on the LocId (station_no) and time series list.
        populateTsShortNameChoices(getSelectedDataStore(), getSelectedDataType(), getSelectedLocId());
        // Now select what the command had previously (if specified).
        //if ( JGUIUtil.isSimpleJComboBoxItem(__TsShortName_JComboBox, TsShortName, JGUIUtil.CHECK_SUBSTRINGS, "-", 1, index, true ) ) {
	    if ( JGUIUtil.isSimpleJComboBoxItem( __TsShortName_JComboBox, TsShortName, JGUIUtil.NONE, null, null ) ) {
            ...
            if ( (TsShortName == null) || TsShortName.equals("") ) {
                // New command...select the default.
                // Populating the list above selects the default that is appropriate so no need to do here.
            	__TsShortName_JComboBox.select (0);
            }
                ...
                // Bad user command.
                Message.printWarning ( 1, routine, "Existing command references an invalid\n"+
                  "TsShortName parameter \"" + TsShortName + "\".  Select a\ndifferent value or Cancel." );
            	__TsShortName_JComboBox.select (0);
            }
        }
		...
		InputFilter_JPanel filterPanel = getVisibleInputFilterPanel();
		if ( filterPanel == null ) {
			Message.printWarning(1, routine, "Trouble finding visible input filter panel for selected KiWIS datastore." );
		}
		else {
	...
	LocId = __LocId_JComboBox.getSelected();
	TsShortName = __TsShortName_JComboBox.getSelected();
	...
	String tsShortName = TsShortName;
	...
	tsid.append ( dataType + "-" + tsShortName );
	tsid.append ( "." );
	if ( (Interval != null) && !Interval.equals("*") ) {
...
For KiWIS there is currently only one input filter per datastore.
@param dataStore the data store from the DataStore and InputName parameters.
*/
private void selectInputFilter ( KiWISDataStore dataStore ) {
    ...
    // Selected data type and interval must be converted to KiWIS internal convention.
    // The following lookups are currently hard coded and not read from KiWIS.
    String selectedDataType = getSelectedDataType();
    ...
    for ( InputFilter_JPanel panel : inputFilterJPanelList ) {
        ...
        KiWISDataStore datastore =
            ((KiWIS_TimeSeries_InputFilter_JPanel)panel).getDataStore();
