...
getPluginVersion() {
  ...
  # Maven folder structure results in duplicate 'owf-tstool-kiwis-plugin'?
  # TODO smalers 2022-05-19 need to enable this.
  srcFile="${repoFolder}/owf-tstool-kiwis-plugin/src/main/java/org/openwaterfoundation/tstool/plugin/kiwis/PluginMeta.java"  
...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Copy the TSTool KiWIS plugin installer files to the Amazon S3 static website folder:"
...
uploadInstaller() {
  ...
  installerUploadCount=0
    ...
    if [ "$includeWindows" = "yes" ]; then
      ...
      zipFilePattern="${distFolder}/tstool-kiwis-plugin-${pluginVersion}-win-*.zip"
...
logInfo ""
logInfo "${installerUpdateCount} TSTool KiWIS plugin installers were uploaded to Amazon S3 location:"
logInfo ""
logInfo "  ${s3FolderUrl}"
logInfo "  ${gpDownloadUrl}"
logInfo ""
logInfo "Next steps are to do the following:"
logInfo "  - Visit the following folder to download the TSTool KiWIS plugin:"
logInfo "      ${gpDownloadUrl}/"
logInfo ""
