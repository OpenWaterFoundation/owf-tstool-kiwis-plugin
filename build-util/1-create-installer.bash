...
#   ./tstool/NN/plugins/owf-tstool-kiwis-plugin/
# - output is to the plugin 'dist' folder with filename like:
#     tstool-kiwis-plugin-1.0.0-win-2206060102.zip
...
createPluginZipFile() {
  ...
  "${sevenzip}" a -tzip ${zipFile} owf-tstool-kiwis-plugin
  exitStatus=$?
...
now=$(date +%Y%m%d%H%M)
zipFile="${distFolder}/tstool-kiwis-plugin-${pluginVersion}-win-${now}.zip"
