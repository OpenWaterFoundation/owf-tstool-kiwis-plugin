#!/bin/bash
#
# Create the TSTool KiWIS plugin product 'index.html' page on Amazon S3,
...
#
# software.openwaterfoundation.org/tstool-kiwis-plugin/index.html
# software.openwaterfoundation.org/tstool-kiwis-plugin/index.csv
#
...
#   2018-12-04 16:16:22          0 tstool-kiwis-plugin/
#   2018-12-04 16:16:37          0 tstool-kiwis-plugin/1.0.0/
#   2018-12-04 16:17:19   46281975 tstool-kiwis-plugin/1.0.0/tstool-kiwis-plugin-win-1.0.0-2201020304.zip
createCatalogFile() {
  ...
  if [ ${errorCode} -ne 0 ]; then
    logError ""
    logError "Error listing TSTool KiWIS plugin download files to create catalog (maybe missing --aws-profile)?"
    exit 1
  fi
  ...
  tmpS3CatalogPath="/tmp/${USER}-tstool-kiwis-plugin-catalog-ls-installers.txt"
  # Only list installers that match the current download filename specification
  cat ${tmpS3ListingPath} | grep -E 'tstool-kiwis-plugin.*.zip' > ${tmpS3CatalogPath}
  logInfo "Available TSTool KiWIS plugin installers:"
...
createIndexHtmlFiles() {
  ...
  indexHtmlTmpFile="/tmp/${USER}-tstool-kiwis-plugin-index.html"
...
<title>TSTool KiWIS Plugin Downloads</title>
</head>' >> ${indexHtmlTmpFile}
...
<li>The TSTool KiWIS Plugin requires that TSTool is also installed if not already installed:
    <ul>
...
<ol>
<li> Download a TSTool KiWIS Plugin Windows installer (zip file) from below.</li>
<li> Follow the latest <a href="https://software.openwaterfoundation.org/tstool-kiwis-plugin/latest/doc-user/appendix-install/install/">installation instructions</a>
    ...
    cat "${tmpS3CatalogPath}" | grep "${downloadOs}-" | awk '{ printf "%s %s %s %s\n", $1, $2, $3, $4 }' | sed -E 's|([0-9][0-9]/)|\1-zzz|g' | sed 's|/-zzz|-zzz|g' | sed 's|dev|-dev|g' | sort -r -k4,4 | sed 's|-zzz||g' | sed 's|-dev|dev|g'  | awk '
      {
        ...
        downloadFileProduct=""            # The product name (e.g., TSTool KiWIS Plugin)
        ...
        if ( filenameSpecVersion = 1 ) {
          # Initial filename spec using format:  tstool-kiwis-plugin/1.0.0.dev/software/tstool-kiwis-plugin-1.0.0.dev-win-2201020304.zip
          ...
          downloadFileProduct="tstool-kiwis-plugin"
          downloadFileVersion=downloadFileParts[4]
          downloadFileOs=downloadFileParts[5]
        }
        # Set the URL to the download file.
        downloadFileUrl=sprintf("https://software.openwaterfoundation.org/tstool-kiwis-plugin/%s/software/%s", downloadFileVersion, downloadFile)
        product = "TSTool KiWIS Plugin"
...
printUsage() {
  ...
  echoStderr ""
  echoStderr "Create the software.openwaterfoundation.org/tstool-kiwis-plugin/index.html and related files."
...
printVersion() {
  ...
  echoStderr ""
  echoStderr "TSTool KiWIS Plugin"
...
uploadIndexFiles() {
  ...
  logInfo "Assuming that TSTool KiWIS Plugin installer files were previously uploaded to S3."
  ...
  if [ -f "${s3CatalogTxtFileUrl}" ]; then
    ...
    if [ ${errorCode} -ne 0 ]; then
      logError ""
      logError "Error uploading TSTool KiWIS Plugin catalog file."
      # Not fatal.
    fi
  else
  ...
  if [ "${operatingSystem}" = "mingw" ]; then
    ...
    MSYS_NO_PATHCONV=1 ${awsExe} cloudfront create-invalidation --distribution-id "${cloudFrontDistributionId}" --paths '/tstool-kiwis-plugin/index.html' '/tstool-kiwis-plugin/' '/tstool-kiwis-plugin' "/tstool-kiwis-plugin/${indexFaviconFile}" --output json --profile "${awsProfile}" | tee ${tmpFile}
  else
...
s3FolderUrl="s3://software.openwaterfoundation.org/tstool-kiwis-plugin"
gpDownloadUrl="https://software.openwaterfoundation.org/tstool-kiwis-plugin"
...
user=""
tmpS3ListingPath="/tmp/${USER}-tstool-kiwis-plugin-catalog-ls.txt"
